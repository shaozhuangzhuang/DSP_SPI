...
参数 psController 指定一个 \e tIpcController 实例的地址，该实例用于存储有关“Put”和“Get”循环缓冲区及其各自索引的信息。

参数 usCPU2IpcInterrupt 指定 psController 使用的 CPU2 IPC 中断号。

参数 usCPU1IpcInterrupt 指定 psController 使用的 CPU1 IPC 中断号。

此函数使用 IPC 中断对的循环缓冲区和索引地址初始化 IPC 驱动程序控制器。 \e usCPU2IpcInterrupt 和 \e usCPU1IpcInterrupt 参数可以是以下值之一：
\b IPC_INT0、\b IPC_INT1、\b IPC_INT2、\b IPC_INT3。

注意如果一个中断当前正在被一个\e tIpcController 实例使用，则该特定中断不应绑定到第二个\e tIpcController 实例。

注意 对于特定的 usCPU2IpcInterrupt - usCPU1IpcInterrupt 对，必须在 CPU1 和 CPU2 系统上定义和初始化 tIpcController 的实例。
