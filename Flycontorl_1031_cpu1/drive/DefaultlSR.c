...
unsigned int timer_500us = 0;
unsigned int TIMER0_1MS_1 = 0;
#ifdef CPU2
extern unsigned int tdataA[36];
#endif
//
...
{
    Uint16 i;
    if(SCIC_Tx_State.Field_Number == One)//第一段16字节发送
     {
         for(i=0; i< 16; i++)
         {
             ScicRegs.SCITXBUF.all = SCIC_SEND_DATALINK[i];  // Send data
         }
         SCIC_Tx_State.Field_Number = Two;
         ScicRegs.SCIFFTX.bit.TXFFINTCLR = 1;
     }
     else if(SCIC_Tx_State.Field_Number == Two)//第二段16字节发送
     {
         for(i=16; i< 32; i++)
         {
             ScicRegs.SCITXBUF.all = SCIC_SEND_DATALINK[i];  // Send data
         }
         SCIC_Tx_State.Field_Number = Three;
         ScicRegs.SCIFFTX.bit.TXFFINTCLR = 1;
     }
     else if(SCIC_Tx_State.Field_Number == Three)//第三段16字节发送
     {
         for(i=32; i< 48; i++)
         {
             ScicRegs.SCITXBUF.all = SCIC_SEND_DATALINK[i];  // Send data
         }
         SCIC_Tx_State.Field_Number = Four;
         ScicRegs.SCIFFTX.bit.TXFFINTCLR = 1;
     }
     else if(SCIC_Tx_State.Field_Number == Four)//第四段16字节发送
     {
         for(i=48; i< 64; i++)
         {
             ScicRegs.SCITXBUF.all = SCIC_SEND_DATALINK[i];  // Send data
         }
         SCIC_Tx_State.Field_Number = Five;
         ScicRegs.SCIFFTX.bit.TXFFINTCLR = 1;
     }
     else if(SCIC_Tx_State.Field_Number == Five)//第五段16字节发送
     {
         for(i=64; i< 80; i++)
         {
            ScicRegs.SCITXBUF.all = SCIC_SEND_DATALINK[i];  // Send data
         }
         SCIC_Tx_State.Field_Number = Zero;
         SCIC_Tx_State.State = Stop_Tx;
     }
...
{
#ifdef CPU2
    static unsigned int TX_number = 0;
    ...
    switch( TX_number )
    {
        case 0:
            for (i = 0; i < 16; i++)
                SciaRegs.SCITXBUF.all = tdataA[i];
        ...
        case 1:
            for (i = 0; i < 16; i++)
                ScidRegs.SCITXBUF.all = tdataA[i + 16];
        ...
        case 2:
            for (i = 0; i < 4; i++)
                ScidRegs.SCITXBUF.all = tdataA[i + 32];
...
//
extern unsigned int SCIA_Txdate[48];
interrupt void SCIA_TX_ISR(void)
{
    Uint16 i;
     if(SCIA_Tx_State.Field_Number == One)//第一段16字节发送
     {
         for(i=0; i< 16; i++)
         {
            SciaRegs.SCITXBUF.all = SCIA_Txdate[i];  // Send data
         }
         SCIA_Tx_State.Field_Number = Two;
         SciaRegs.SCIFFTX.bit.TXFFINTCLR = 1;
     }
     else if(SCIA_Tx_State.Field_Number == Two)//第二段16字节发送
     {
         for(i=16; i< 32; i++)
         {
            SciaRegs.SCITXBUF.all = SCIA_Txdate[i];  // Send data
         }
         SCIA_Tx_State.Field_Number = Three;
         SciaRegs.SCIFFTX.bit.TXFFINTCLR = 1;
     }
     else if(SCIA_Tx_State.Field_Number == Three)//第三段16字节发送
     {
         for(i=32; i< 48; i++)
         {
            SciaRegs.SCITXBUF.all = SCIA_Txdate[i];  // Send data
         }
         SCIA_Tx_State.Field_Number = Zero;
         SCIA_Tx_State.State = Stop_Tx;
     }
     PieCtrlRegs.PIEACK.all |= PIEACK_GROUP9;       // Issue PIE ACK
}
