...
// 外部声明DMA ISR（在drv_dma.c中定义）
extern __interrupt void DMA_CH1_ISR(void);
extern __interrupt void DMA_CH2_ISR(void);
extern __interrupt void DMA_CH5_ISR(void);
extern __interrupt void DMA_CH6_ISR(void);

// 外部声明ADS1278 DRDY ISR（在drv_ads1278.c中定义）
extern __interrupt void ADS1278_DRDY_ISR(void);

/**
 * @brief  配置SPI+DMA相关中断（PIE向量表与中断使能）
 * @note   配置DMA CH1/2/5/6（PIE7.1/7.2/7.5/7.6）和DRDY XINT2（PIE1.5）
 */
void Drv_Interrupt_SPI_DMA_Init(void)
{
    EALLOW;
    
    //=========================================================================
    // 配置DMA中断向量（PIE Group 7）
    //=========================================================================
    PieVectTable.DMA_CH1_INT = &DMA_CH1_ISR;    // PIE7.1 - SPIA RX
    PieVectTable.DMA_CH2_INT = &DMA_CH2_ISR;    // PIE7.2 - SPIA TX
    PieVectTable.DMA_CH5_INT = &DMA_CH5_ISR;    // PIE7.5 - SPIB TX
    PieVectTable.DMA_CH6_INT = &DMA_CH6_ISR;    // PIE7.6 - SPIB RX
    
    //=========================================================================
    // 配置DRDY中断向量（PIE Group 1）
    //=========================================================================
    PieVectTable.XINT2_INT = &ADS1278_DRDY_ISR; // PIE1.5 - DRDY (XINT2)
    ...
    // 使能PIE Group 1（XINT2 - DRDY）中断
    PieCtrlRegs.PIEIER1.bit.INTx5 = 1;          // XINT2
    
    //=========================================================================
    // 使能CPU级中断
    //=========================================================================
    IER |= M_INT7;  // 使能CPU INT7（PIE Group 7 - DMA）
    IER |= M_INT1;  // 使能CPU INT1（PIE Group 1 - XINT1）
