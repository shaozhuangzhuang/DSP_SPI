...
//
    ...
    {
        //串口A GPIO初始化
        GPIO_SetupPinMux(135, GPIO_MUX_CPU2, 6); // GPIO135 - (SCITXDA)
        GPIO_SetupPinMux(136, GPIO_MUX_CPU2, 6); // GPIO136 - (SCIRXDA)
        GPIO_SetupPinOptions(136, GPIO_INPUT, GPIO_PUSHPULL );
        GPIO_SetupPinOptions(135, GPIO_OUTPUT, GPIO_ASYNC);

        GPIO_SetupPinMux(98,GPIO_MUX_CPU1,0);     // DE_A2
        GPIO_SetupPinOptions(98, GPIO_OUTPUT,1);
        GpioDataRegs.GPDSET.bit.GPIO98 = 1;
        GPIO_SetupPinMux(99,GPIO_MUX_CPU1,0);     // /RE_A2
